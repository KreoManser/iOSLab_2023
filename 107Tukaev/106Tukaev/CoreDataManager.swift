...
class CoreDataManager {
    ...
    func createFetchedSubPublication() -> NSFetchedResultsController<Publication> {

        let publFetchRequest = Publication.fetchRequest()
        let predicate = NSPredicate(format: "user IN %@", argumentArray: [authUser?.subscription ?? User()])
        publFetchRequest.predicate = predicate
        let sortDescriptor = NSSortDescriptor(key: "date", ascending: true)
        publFetchRequest.sortDescriptors = [sortDescriptor]

        let resultController = NSFetchedResultsController(
            fetchRequest: publFetchRequest,
    ...
    func createFetchedUserPubl(user: User) -> NSFetchedResultsController<Publication> {
