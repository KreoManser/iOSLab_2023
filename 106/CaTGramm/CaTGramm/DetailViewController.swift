...
class DetailViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    lazy var previousController = ViewController()

    lazy var datasource = DataManager.OurDataManager.dataSource
...
extension DetailViewController: PostTableAlertDelegate {
    ...
    func presentAlert(indexPath: IndexPath) {
        ...
        alert.addAction(UIAlertAction(title: "Удалить", style: .destructive, handler: {  [weak self] _ in
            DataManager.OurDataManager.asyncDeleteModel((self?.datasource[indexPath.row]) ?? (DataManager.OurDataManager.dataSource[0]))
