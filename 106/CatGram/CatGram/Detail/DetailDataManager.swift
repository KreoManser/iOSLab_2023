...
class DetailDataManager: NSObject, UITableViewDataSource, UITableViewDelegate, DetailTableViewCellDelegate {
    func didDeletePublication(publication: DetailPublication) {
        profileDataManager.asyncDelete(ProfilePublication(id: publication.id, image: publication.postImage,
        caption: publication.caption, date: publication.date, isFavorite: publication.isFavorite), completion: { result in
            switch result {
            case .success:
        ...
        dataManager.asyncGetAllPosts { result in
            ...
            case .success(let publications):
                for publication in publications {
                    self.detailPublication.append(DetailPublication(id: publication.id, postImage: publication.image, caption: publication.caption,
                        date: publication.date, isFavorite: publication.isFavorite))
                }
            case .failure(let error):
                print("Ошибка получения постов: \(error)")
            }
